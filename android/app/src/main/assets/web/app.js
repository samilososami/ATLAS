...
async function openTerminal(){initTerminal();if(terminalId)return;terminalInput.cancel();const t=await native('terminal.open',{cols:terminal.cols,rows:terminal.rows});terminalId=t.terminal;$('#terminal-status').textContent='Conectada';terminal.focus();pollTerminal();}
...
async function closeTerminal(){clearTimeout(terminalTimer);terminalInput.cancel();const id=terminalId;terminalId=null;$('#terminal-status').textContent='Desconectada';if(id)try{await native('terminal.close',{terminal:id});}catch{}}
...
$$('#terminal-keys button').forEach(b=>b.onclick=safe(()=>{if(terminalId)terminalInput.push(keys[b.dataset.key]);terminal?.focus();}));
