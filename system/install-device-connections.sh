...
for path in /usr/local/bin/atlas-audio /usr/local/bin/atlas-say /usr/local/bin/adb \
...
install -d -m 700 -o "$atlas_user" -g "$atlas_group" "$atlas_home/.atlas/runtime/tmp"
install -m 755 "$repo/atlas-commands/atlas-audio" /usr/local/bin/atlas-audio
install -m 755 "$repo/atlas-commands/atlas-say" /usr/local/bin/atlas-say
