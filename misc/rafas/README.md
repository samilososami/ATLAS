...
Or use `atlas-screen --rafas --on` (`--RAFAS` is also accepted). Like the other
screen modes, `--rafas` alone selects it without turning on the screen.
To leave, use `atlas-screen --atlas --on`, `--desktop --on`, `--terminal --on`
or `atlas-screen off`. The selected mode changes, but the display still boots off.
...
Neither the activation service nor the root console is enabled at boot.
