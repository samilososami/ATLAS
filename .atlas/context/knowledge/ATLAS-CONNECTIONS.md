...
`/home/atlas/.atlas/context/knowledge`.

| Layer | Public implementation | Operational manual |
| --- | --- | --- |
| HTTP service and provider readiness | `.atlas/webscreen/server.py`, `.atlas/broker/native_broker.py`, `system/systemd/atlas-webscreen.service` | [Broker](atlas-commands/ATLAS-BROKER.md), [WebScreen](atlas-commands/ATLAS-WEBSCREEN.md), [runtime source guide](../../webscreen/README.md) |
...
| Calibrated double applause | `.atlas/webscreen/static/clap.js`, shared `app.js` analyser hook, `.atlas/webscreen/server.py` profile endpoint | [Double applause](../../webscreen/CLAP.md) |
...
| Minimal face, waveform and mouth | `.atlas/webscreen/static/new/`, shared `app.js` / `realtime.js` hooks | [New design](../../webscreen/NEW_DESIGN.md), `atlas-screen --atlas-new` |
| Same model without voice | `.atlas/chat/atlas_chat.py`, `.atlas/chat/TERMINAL_INSTRUCTIONS.md` | [atlas-chat](atlas-commands/ATLAS-CHAT.md), [chat runtime](../../chat/README.md) |
...
| Declarative role boundaries | `.atlas/roles/*/role.json`, `.atlas/roles/role.schema.json` | [Roles](../../roles/README.md), [professors demo](../../roles/profesores/PROFESORES.md) |
...
The Native Broker supplies the authentication/reservation path through one
persistent `codex app-server`: Codex refreshes its own OAuth, the broker reads a
normalised quota snapshot and only short-lived Realtime credentials leave the
backend. Realtime handles the conversation directly with shared Markdown and
explicit tools. The archived OpenClaw/prelude pipeline is historical evidence,
not an automatic fallback or an installed runtime dependency.
Role manifests are also not a transport or recovery layer. The current
WebScreen and `atlas-chat` sessions use the complete `atlas-full`-equivalent
context; dynamic role selection/composition remains future work.
...
- The HTTP server remains available while Native Broker, OAuth or provider
  recovery runs in the background. `/api/health` is a snapshot, not a blocking reconnect operation;
...
3. For local connectivity, distinguish HTTP access, browser ownership and Native
   Broker readiness. For provider errors, retain the error category/request ID without
...
- `sudo bash system/install-webscreen-resilience.sh`: the shared WebScreen and
  Native Broker files: `server.py`, `access_control.py`, `.atlas/broker/`, `static/access.js`,
  ...
  configuration/OAuth/context and does not restart anything by default. It does
  not install or enable OpenClaw.
  `--restart` restarts only `atlas-webscreen.service`; reload browser tabs after
  deployment. See the [runtime guide](../../webscreen/README.md).
