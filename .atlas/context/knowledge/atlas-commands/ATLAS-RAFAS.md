...
It also validates the standalone knowledge, conversation, runtime-temp and
private `secrets.json` paths without exposing secret values.
...
desktop and the graphical terminal are stopped; the ATLAS backend and network
are left alone. `atlas-screen` and `atlas-status` show the active mode.
