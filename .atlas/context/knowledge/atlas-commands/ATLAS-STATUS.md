...
`atlas-status` normally takes around 1.5 to 2 seconds because it intentionally collects a broad snapshot: temperature, storage, RAM, CPU, fan, throttling, standalone ATLAS runtime, WebScreen, desktop, cast and physical screen state.
...
- top RAM consumer
- standalone ATLAS runtime layout and private-config permissions
...
```text
Atlas Runtime: ready / knowledge+conversation+runtime+secrets
...
That means the standalone runtime layout is ready, but visual/cast surfaces are not consuming resources.
