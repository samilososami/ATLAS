...
its Native Broker, Codex OAuth or provider session is recovering. A healthy backend also does not prove that
...
IDs. Check network, page ownership, Native Broker/OAuth, microphone and
...
It should not stop the Native Broker. `disable` is for the WebScreen service,
not the full ATLAS runtime.
